...
all_patient_sum = 0
all_patient_sum_res = 0

while (patient_num <= 130):
    y1 = []
    y2 = []
    x = []
    with tf.Session() as sess:
        ...
        for i in range(1, numberofFile+1):
            ...
            y_2 = res[0]
            if res[0] < 0.9:
                matrix_output = fillHole.optimizer_CT_Blur(output_path)
                res = sess.run([dice, y_pred_sum, y_true_sum],
                               feed_dict={tf_matrix_label: matrix_label, tf_matrix_output: matrix_output})
            sum += res[0]
            sum_res += y_2
        ...
        print_sum= sum / numberofFile
        print_sum_res = sum_res /numberofFile

        print (print_sum)
        print (print_sum_res)

        all_patient_sum += print_sum
        all_patient_sum_res += print_sum_res
        #plt.plot(x, y, 'r')
        plt.figure(figsize=(20, 10))
        plt.plot(x, y1, 'r', lw=1, marker='o')
        plt.plot(x, y2, 'b', lw=1)
        plt.savefig('/home/chacky/PycharmProjects/untitled3/Doc/Patient_Blur_'+str(patient_num)+'.jpg')
        plt.close()
        patient_num+=1

print (all_patient_sum / 26)
print (all_patient_sum_res /26)
#93
#