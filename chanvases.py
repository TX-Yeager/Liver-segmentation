...
def convergence(p_mask, n_mask, thresh, c):
    # diff = p_mask - n_mask
    diff = np.subtract(p_mask, n_mask, dtype=np.float32)
...
if __name__ == "__main__":
    #img = nd.imread("chanv_test.png", flatten=True)
    img = nd.imread("576.png", flatten=True)
    #img = nd.imread("./horizontal_plane/573.png", flatten=True)

    mask = np.zeros(img.shape)
    #mask[180:200, 380:420] = 1
    mask[100:450, 200:420] = 1
    chanvese(img, mask, max_its=1500, display=True, alpha=1.0,thresh=0)
