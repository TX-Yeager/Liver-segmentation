...
with tf.Session() as sess:
    ...
    output_dir_CRF = "./result/" + str(patient_num) + "DensecrfMask_blur/"
    ...
    for i in range(413, 598):
        ...
        output_path_CRF, label_path = next_pic(output_dir_CRF, label_dir, i)
        ...
        matrix_output_CRF, matrix_label = prepare_dice_CT_pic(output_path_CRF, label_path)
        #label
        ...
        entropy = sess.run(loss,
                           feed_dict={tf_matrix_label: matrix_label, tf_matrix_output: matrix_output})
        loss_sum += entropy
        res_CRF = sess.run([dice, y_pred_sum, y_true_sum],
                   feed_dict={tf_matrix_label: matrix_label, tf_matrix_output: matrix_output_CRF})
        y3.append(res_CRF[0])
        #processMask
        res = sess.run([dice, y_pred_sum, y_true_sum],
        ...
        y2.append(y_2)
    numberofFile = 185
    print("Label",sum1/numberofFile)
    print("Process",sum/numberofFile)
    print("CRF", sum2 / numberofFile)
    ...
    ax1.plot(x, y3, 'black', lw=1, marker='.', label="CRF")
    ax1.plot(x, y1, 'r', lw=1, marker='.', label="Process")
    ...
    whichisbetter = np.where(y3 > y1, y3, y1)
    which = []
    whichis = []

    for i in range(0, len(y3)):
        matrix_comp=[y3[i],y1[i],y2[i]]
        dice = max(matrix_comp)
        pos = np.where(matrix_comp==dice)
        pos = np.array(pos, dtype=int)
        whichis.append(pos[0][0])
        which.append(dice)

    plt.figure(1,figsize=(10, 5))

    plt.subplot(2, 1, 1)
    plt.plot(x, which, 'r', lw=1, marker='.',label="best")
    plt.plot(x, y2, 'g', lw=1,marker='.',label="Only FCN")

    plt.subplot(2, 1, 2)
    scale_ls = range(0,3)
    index_ls = ['CRF', 'Process', 'Only FCN']
    plt.yticks(scale_ls,index_ls)
    plt.plot(x, whichis, 'g', lw=1, marker='.', label="which")
    print(np.sum(which)/numberofFile)
